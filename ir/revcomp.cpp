...
std::unique_ptr<dsl::codegen::ValueBuilder> KmerRevcomp::getBuilder() const {
  return std::make_unique<LLVMRevcomp>(kmer);
}

std::unique_ptr<dsl::codegen::CFBuilder> KmerRevcomp::getCFBuilder() const {
...
Value *KmerRevcomp::doClone(util::CloneVisitor &cv) const {
